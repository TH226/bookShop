$(function() {
	...
	$(".inputClass").focus(function(){
		var lableId = $(this).attr("id")+"Error"; //通过输入框找到lable的id
		$("#"+lableId).text("");//lable内容清空
		showError($("#"+lableId));//隐藏没有错误信息的页面
	});
	/**
	 * 输入框失去焦点进行校验
	 */
	$(".inputClass").blur(function(){
		var id = $(this).attr("id");//获取当前失去焦点的输入框id
		var funName = "validate"+id.substring(0,1).toUpperCase()+id.substring(1)+"()";//得到对应的校验方法
		eval(funName);//执行函数调用
	});
});
/**
