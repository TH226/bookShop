...
$(function(){
	...
	*/
	$(":checkbox[name=checkboxBtn]").click(function(){
		var all = $(":checkbox[name=checkboxBtn]").length;//所有条目个数
		var select = $(":checkbox[name=checkboxBtn][checked=true]").length;//获取所有被选条目个数
		if(all==select){
			$("#selectAll").attr("checked",true);//勾选全选复选框
			setJieSuan(true);//让结算按钮有效
		}else if(select==0){
			$("#selectAll").attr("checked",false);//勾选全选复选框
...
function showTotal(){
	var total = 0;
	/*
	1.获取所有被勾选的条目复选框,循环遍历
	*/
	$(":checkbox[name=checkboxBtn][checked=true]").each(function(){
		//2.获取复选框的值，即cartItemID 其他元素的前缀
		var id = $(this).val();
		//3.再通过cartItemID找到小计，获取其文本
...
<c:forEach items="${cartItemList }" var="cartItem">
	<tr align="center">
		...
		<td align="left" width="70px">
			<a class="linkImage" href="<c:url value='/BookServlet?method=load&bid=${cartItem.book.bid }'/>"><img border="0" width="54" align="top"
		...
		<td>
			...
			id="${cartItem.cartItemId }Quantity" type="text" value="${cartItem.quantity }"/><a class="jia" 
